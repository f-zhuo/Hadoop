...
* Map：
  ...
  从hdfs输出的数据经过数据分割（input split）变成若干等长的数据块block（压缩文件除外，压缩文件不可以被split，一个压缩文件对应一个map），block传给mapper（一个split对应一个map），mapper把其写入内存，同时，把block数据转换为record。内存默认大小是100M，当写入80%时，把内存中的数据输出并存储在磁盘里（溢写/spill）。存储时，会按照key进行排序，把相同的key放在一起。因为数据循环写入内存，放入磁盘，故当数据再次放入磁盘时，这些按照key排好序的数据和之前存入磁盘的数据组成了一个个分区（partition）。内存缓存数据时，写入的数据就是（partition,key,value）,value是对key的一次计数
...
* reduce：
  ...
  每个reducer从所有mapper的输出中寻找特定的key（之前提到的哈希），且只会对这一个/几个特定key的数据块进行merge（合并）计算，得到最终结果。MapReduce的输出文件数等于reducer数，输出分为key和value两个部分，默认是以空格为分割符（saparator）的，以分割的第一个字段为key，默认以key作为partition

*归并和合并的区别*

对于(apple,1)，(apple,1)这两个数据，
...
shuffle过程复杂，占用资源，为了优化，可以使用combine。所谓combine，就是在map过程中提前进行"reduce"

combine会在每个mapper中进行reduce的操作，也就是按键聚合。最终reducer从每个mapper中得到的输入，相同键的键值对只有一个，简化了shuffle复杂度。但是应该慎用，有些情况不可用
...
`mapper1：(0001,34)，(0001,27)`

`mapper2：(0001,57)，(0001,39)`

`reduce：(0001,[34,27,57,39])-->(0001,157)`
...
`reduce：(0001,[61,96])-->(0001,157)`

这两种方法计算都没有问题，但是如果是计算平均值，combine就会出错
...
`reduce：(0001,[4.95.12.73])-->(0001,17.68)`

而实际值是22.2

## MapReduce的作业提交

提交MapReduce作业需要四个文件和streaming架构

* run.sh文件：包含streaming架构的文件，也是提交作业需要运行的文件
...
jobtracker：主进程，客户端发送请求给jobtracker，主进程分配资源并调度给工作进程tasktracker，通过tasktracker发送的心跳heartbeat监控其工作状态和任务进度。一旦某个tasktracker工作失败，就把作业交给其他tasktracker。主进程采用进程池的方法处理用户请求和监控，一个MapReduce集群只有一个主进程
...
* 指定优先级：从高到低，Very_high-high-normal-low-very_low
